...
Here is a guide to debugging Electron App with VSCode. It works on all platforms, and includes
...
Clone and open the project in VSCode. Then go to the Debug Viewlet and click the configure icon
...
If you go to the Debug Viewlet and run `Debug Main Process`, you should see the code stopping
...
and VSCode has an extension that just handles that: [Debugger for Chrome]
...
```
Remote debugging doesn't work with multiple DevTools clients. We'll use the debugger in VSCode
...
generated during transpilation, we can debug the original code in VSCode.
...
But I find myself debugging much faster in VSCode, which I use everyday for coding.
